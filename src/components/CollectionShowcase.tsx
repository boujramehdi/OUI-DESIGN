    ...
    <section className="bg-ivory text-charcoal">
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-16 lg:py-28">
        ...
        <div className="reveal mb-8 flex flex-col gap-4 sm:mb-12 sm:flex-row sm:items-end sm:justify-between lg:mb-16">
          <div>
            <p className="text-[0.62rem] uppercase tracking-[0.4em] text-bronze">Collection Oui Design</p>
        ...
        <div className="lg:hidden">
          ...
          <div className="mb-4 flex items-center justify-between">
            <span className="text-[0.44rem] uppercase tracking-[0.38em] text-charcoal/30">
              {collectionItems.length} créations
            </span>
            <div className="flex items-center gap-1.5">
              {collectionItems.map((_, i) => (
                <div key={i} className="h-px w-4 bg-bronze/25" />
              ))}
              <span className="ml-1 text-[0.42rem] uppercase tracking-[0.32em] text-charcoal/25">Glisser</span>
            </div>
          </div>
                  ...
                  <Link href={`/collection/${item.slug}`} className="group block">
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden bg-charcoal">
                      <Image
                        ...
                        fill
                        sizes="82vw"
                        className={`transition-transform duration-700 group-active:scale-105 ${collectionImages[index]?.fit}`}
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-charcoal/65 via-charcoal/10 to-transparent" />

                      {/* Price + corner deco */}
                      <div className="absolute right-0 top-0 h-10 w-px bg-bronze/30" />
                      <div className="absolute right-0 top-0 h-px w-10 bg-bronze/30" />
                      <div className="absolute bottom-4 left-4">
                        <p className="text-[0.5rem] uppercase tracking-[0.3em] text-bronze/90">{item.price}</p>
                      </div>
                    </div>

                    {/* Info strip */}
                    <div className="mt-4 flex items-start gap-4 border-l-2 border-bronze/30 pl-4 transition-all duration-300 group-hover:border-bronze">
                      <span className="shrink-0 font-serif text-2xl leading-none text-bronze/40">
                        {String(index + 1).padStart(2, "0")}
                      </span>
                      <div className="min-w-0">
                        <h3 className="text-[0.62rem] font-medium uppercase tracking-[0.2em] text-charcoal">
                          {item.title}
                        </h3>
                        <p className="mt-1.5 text-[0.72rem] leading-[1.6] text-charcoal/50">
                          {item.description.slice(0, 85)}…
                        </p>
                        <p className="mt-3 inline-flex items-center gap-2 text-[0.5rem] uppercase tracking-[0.22em] text-bronze transition-all duration-300 group-hover:gap-3">
        ...
        <div className="hidden lg:grid lg:grid-cols-[1.7fr_1fr] lg:gap-10 lg:items-stretch">
          ...
          <div className="reveal relative aspect-[3/2] overflow-hidden bg-charcoal sm:aspect-[16/10]">
            {collectionImages.map((item, i) => (
              <div
                ...
                className={`absolute inset-0 transition-all duration-700 ${
                  i === selected ? "opacity-100 scale-100" : "opacity-0 scale-[1.04]"
                }`}
              >
                <Image
                  src={item.src}
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-charcoal/60 via-charcoal/20 to-transparent px-6 pb-6 pt-16">
              <p className="text-[0.55rem] uppercase tracking-[0.3em] text-bronze/80">{activeItem.price}</p>
              <p className="mt-1 font-serif text-lg text-ivory leading-tight">{activeItem.title}</p>
              <Link
            ...
            {collectionItems.map((item, index) => (
              <Link
                ...
                className={`group relative flex flex-col overflow-hidden border-b border-charcoal/8 py-5 text-left transition-all duration-300 first:border-t first:border-charcoal/8 ${
                  selected === index ? "pl-4 text-charcoal" : "pl-0 text-charcoal/40 hover:pl-2 hover:text-charcoal"
                }`}
              >
                <div
                ...
                <div className="flex items-center justify-between">
                  <span className={`mr-3 shrink-0 font-serif text-lg leading-none transition-colors duration-300 ${
                    selected === index ? "text-bronze" : "text-bronze/30 group-hover:text-bronze/60"
                  }`}>
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <span className="flex-1 text-[0.58rem] font-medium uppercase tracking-[0.22em]">{item.title}</span>
                  <span className={`ml-3 shrink-0 transition-all duration-300 ${
                    selected === index ? "translate-x-0 text-bronze" : "translate-x-1 text-charcoal/20 group-hover:translate-x-0 group-hover:text-bronze/60"
                  }`}>→</span>
                </div>
                <div className={`overflow-hidden transition-all duration-500 ${
                  selected === index ? "mt-3 max-h-24 opacity-100" : "max-h-0 opacity-0"
                }`}>
                  <p className="pl-7 text-[0.58rem] leading-5 text-charcoal/55">{item.description.slice(0, 100)}…</p>
                  <p className="mt-2 pl-7 text-[0.52rem] font-medium uppercase tracking-[0.2em] text-bronze/70">{item.price}</p>
                </div>
              </Link>
            ))}
