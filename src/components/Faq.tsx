        ...
        <div className="mt-10 divide-y divide-bronze/15 border-y border-bronze/15">
          {items.map((item) => (
            <details key={item.question} className="group py-6">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-serif text-xl text-charcoal transition-colors duration-300 hover:text-bronze sm:text-2xl">
              ...
              <p className="mt-5 max-w-3xl pl-4 text-sm leading-7 text-charcoal/65">{item.answer}</p>
            </details>
          ))}
        </div>
      </div>
